...
const Profile = ({ onNavigate, onLogout, user: currentUser }) => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const [userData, notificationsData, addressesData] = await Promise.all([
    ...
    if (!user) {
        if (loadingError) {
            return (
                <div style={{ padding: '40px', textAlign: 'center' }}>
                    ...
                    <p>Pode haver um problema de conexão ou seus dados precisam ser atualizados.</p>
                    <p style={{ fontSize: '0.8rem', color: 'gray' }}>Debug: User ID {currentUser?.id || 'N/A'}</p>
                    <button onClick={onLogout} className="btn-primary" style={{ marginTop: '20px', padding: '12px 24px', borderRadius: '8px' }}>
        ...
        return (
            <div style={{ padding: '40px', textAlign: 'center' }}>
                <h3>Carregando...</h3>
                <p style={{ fontSize: '0.8rem', color: '#999' }}>Aguardando resposta do servidor...</p>
            </div>
        );
    }
